...
## ADR-036 — Patient evidence RAG is a scoped search index

Patient Evidence RAG uses the canonical clinical tables as its sole source of truth and stores only deterministic, source-linked search chunks in `rag_patient_chunks`. Patient and session predicates are applied before ranking; assigned-doctor authorization remains mandatory. PostgreSQL uses pgvector plus full-text indexes, while isolated SQLite tests use the same persisted rows with deterministic in-process scoring. Embedding provider/model/version/dimension are stored per row and incompatible vectors are never silently mixed. Knowledge RAG remains in the physically separate `knowledge_chunks` corpus. Retrieval and deterministic answer rendering cannot diagnose, recommend treatment, promote unverified OCR, resolve conflicts, or block canonical workflows when indexing fails.
