...
it('renders hospitals and requires an explicit visit selection', () => {
  const select = vi.fn();
  render(<HospitalSelection hospitals={[{ id: 'h1', name: 'City Hospital', city: 'Kolkata', address: 'Central Kolkata' }]} load={vi.fn()} busy={false} onSelect={select} />);
...
it('shows loading, empty, and retryable failure states', async () => {
  const loading = render(<HospitalSelection hospitals={null} load={vi.fn().mockResolvedValue(undefined)} busy={false} onSelect={vi.fn()} />);
  expect(screen.getByRole('status')).toHaveTextContent('Loading hospitals');
  loading.unmount();
  const empty = render(<HospitalSelection hospitals={[]} load={vi.fn()} busy={false} onSelect={vi.fn()} />);
