...
import type { EvidenceReference, Summary, SummaryRevisionRecord } from '../../api/client';
...
export default function SummaryWorkspace({
  ...
  if (
    prevSummaryKey !== `${initialSummary.id}-${initialSummary.version}-${initialSummary.status}`
  ) {
    ...
    <section className="card editor summary-workspace" data-testid="summary-workspace">
      ...
      {activeTab === 'evidence' && (
        <div className="summary-tab-content evidence-tab">
          <p className="muted" style={{ marginBottom: '1rem' }}>
            Source attribution mapping. Every summary item traces deterministically to patient
            ...
            >
              {evidenceList.map((ev, idx) => (
                ...
                >
                  ...
                  <p style={{ margin: '0.25rem 0', fontWeight: 600 }}>{ev.statement_text}</p>
              ...
              {revisions.map((rev) => (
                <div
                  ...
                  style={{
                    padding: '0.75rem 1rem',
                    borderLeft:
                      rev.actor_type === 'DOCTOR' ? '4px solid #10b981' : '4px solid #6366f1',
                  }}
                >
                  <div
                    style={{
                  ...
                  <div style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '0.5rem' }}>
                    <span>
                      Actor: <strong>{rev.actor_type}</strong>
                    </span>
                    {rev.actor_name && (
                      <span style={{ marginLeft: '0.5rem' }}>({rev.actor_name})</span>
                    )}
                    {rev.actor_user_id && (
                      <span style={{ marginLeft: '0.5rem' }}>
                        <code>{rev.actor_user_id.slice(0, 8)}</code>
                      </span>
                    )}
                  </div>
                  {rev.review_notes && (
      ...
      {showRegenModal && (
        ...
        >
          ...
          >
            <h3 id="regen-modal-title" style={{ marginTop: 0 }}>
              Confirm Draft Regeneration
            </h3>
            <p style={{ color: '#dc2626', fontWeight: 500 }}>
              ⚠️ Manual edits currently exist on this summary.
            </p>
            <p className="muted" style={{ fontSize: '0.9rem' }}>
              Regenerating will replace your current working draft with a freshly constructed draft
              based on the latest interview answers and medical facts. Your previous edits will
              remain safely preserved in the Revision History.
            </p>
            <div
              style={{
                display: 'flex',
