...
export default async function requireMock() {
  ...
  try {
    ...
    if (!response.ok() || (await response.json()).normalization_provider !== 'mock') {
      throw new Error('Offline E2E requires an explicitly mock backend. Stop dev services, then run scripts/start-dev.ps1 -NormalizationProvider mock. The live evaluator uses its own process.');
    }
  } finally {
