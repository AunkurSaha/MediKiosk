...
foreach ($entry in @(
    ...
    $uri = if ($entry.Kind -eq 'doctor') {
        'http://127.0.0.1:8010/api/doctor/sessions/' + $reference.sessionId
    } else { 'http://127.0.0.1:8010/api/sessions/' + $reference.sessionId + '/interview' }
...
Write-Output ('PASS: ' + $mode + ' restart; seven persisted API snapshots and original document bytes unchanged.')
